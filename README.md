...
- **智能模型选择** — 测试时自动读取 API 模型列表，模型正确则直接跳过，模型不存在或为空时才弹窗供用户选择；也可点击「查询」按钮手动获取
...
| 标签页 | 功能 |
|--------|------|
| 参数设置 | 配置 API 地址/密钥/模型（可选填，支持「查询」按钮获取模型列表）/提示词/并发参数，支持保存/重置/自动保存 |
...
2. 点击「开始测试」，程序自动检测连通性
3. 程序自动验证模型名称：正确则跳过，不存在或为空则弹出列表供选择
4. 先用**并发数=1**测基线延迟
...
model = qwen3.5-122b-a10b-fp8   (选填，可通过「查询」按钮自动获取)
...
模型查询（点击「查询」按钮）失败时也会分类提示：

| 错误类型 | 排查建议 |
|---------|---------|
| 连接失败 | 检查 API 地址、端口、网络/防火墙 |
| 连接超时 | 检查网络连接和 API 地址可达性 |
| 认证失败 (401) | 检查 Key 是否正确、是否过期 |
| 权限不足 (403) | 检查 API 密钥权限和账户配额 |
| 接口不存在 (404) | 确认服务是否支持 /models 接口 |
| 请求限流 (429) | 稍后重试，等待配额恢复 |
| 服务器错误 (5xx) | 稍后重试，联系服务提供方 |
| 响应格式异常 | 确认是否为 OpenAI 兼容接口 |
